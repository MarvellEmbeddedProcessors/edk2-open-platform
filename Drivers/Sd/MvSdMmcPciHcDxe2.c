  ...
  for (Slot = 0; Slot < 1; Slot++) {
    if ((Private->Slot[Slot].Enable) && (Private->Slot[Slot].SlotType == RemovableSlot)) {
      ...
      if ((Status == EFI_MEDIA_CHANGED) && !MediaPresent) {
        DEBUG ((EFI_D_ERROR, "SdMmcPciHcEnumerateDevice: device disconnected at slot %d of pci %p\n", Slot, Private->PciIo));
      ...
      if ((Status == EFI_MEDIA_CHANGED) && MediaPresent) {
        DEBUG ((EFI_D_ERROR, "SdMmcPciHcEnumerateDevice: device connected at slot %d of pci %p\n", Slot, Private->PciIo));
        //
...
//  BOOLEAN                         MediaPresent;
  ...
  //
  // Perform Xenon-specific initialization
  //
  XenonSetMpp ();
  ...
  //
  // Don't get SD/MMC Pci Host Controller Slot info, because it is emulated PCI
  // controller!
  //
/*  Status = SdMmcHcGetSlotInfo (PciIo, &FirstBar, &SlotNum);
  ...
  for (Slot = 0; Slot < (XENON_MMC_SLOT_ID_HYPERION + 1) ; Slot++) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG((DEBUG_ERROR, "JSD: Cannot gather capability\n"));
      continue;
    }
...
/*    Private->Capability[Slot].Sdr104 = 0;
    ...
    if (EFI_ERROR (Status)) {
      DEBUG((DEBUG_ERROR, "JSD: Cannot gather MaxCurrent Value\n"));
      continue;
    }
    ...
    //
    // Don't Reset the specified slot of the SD/MMC Pci Host Controller, since
    // it isn't working well for XENON. Reset function should be changed in
...
/*    Status = SdMmcHcReset (PciIo, Slot);
    ...
    //
    // Don't Check whether there is a SD/MMC card attached, because XENON
    // doesn't support CardDetection.
    //
/*    Status = SdMmcHcCardDetect (PciIo, Slot, &MediaPresent);
    ...
    //
    // Don't init host, since clock is already set properly. XENON doesn't
    // support normal clock changing, it's necessary to adjust its internal
    // registers.
    //
/*    Status = SdMmcHcInitHost (PciIo, Slot, Private->Capability[Slot]);
    ...
    for (Index = 0; Index < RoutineNum; Index++) {
      ...
      if (*Routine != NULL) {
        Status = (*Routine) (Private, Slot);
        if (!EFI_ERROR (Status)) {
          DEBUG((DEBUG_ERROR, "JSD: Routine passed\n"));
          break;
        }
      }
    }
  ...
  DEBUG ((EFI_D_ERROR, "SdMmcPciHcDriverBindingStart: %r End on %x\n", Status, Controller));
  ...
  )
{
  ...
  if (!Private->Slot[Slot].Initialized) {
    DEBUG((DEBUG_ERROR, "JSD: Init flag not set!\n"));
    return EFI_DEVICE_ERROR;
  }
  //
