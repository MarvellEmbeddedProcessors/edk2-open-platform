...
STATIC CONST FVB_DEVICE mMvFvbFlashInstanceTemplate = {
...

  {
    ...
    MvFvbBlockIoReset,                // Reset;
    MvFvbBlockIoReadBlocks,           // ReadBlocks
    MvFvbBlockIoWriteBlocks,          // WriteBlocks
    MvFvbBlockIoFlushBlocks           // FlushBlocks
  }, // BlockIoProtocol
...
{
  DEBUG ((DEBUG_BLKIO,
    "%a: operation not supported (0x%x)\n",
    __FUNCTION__,
    *Attributes));
  ...
  )
{
  ...
  DEBUG ((DEBUG_BLKIO,
    "%a: (Lba=%ld, BlockSize=0x%x, LastBlock=%ld)\n",
    __FUNCTION__,
    Lba,
  ...
  if (Lba > FlashInstance->Media.LastBlock) {
    ...
    *NumberOfBlocks = (UINTN)(FlashInstance->Media.LastBlock - Lba + 1);
    ...
    DEBUG ((DEBUG_BLKIO,
      "%a: *BlockSize=0x%x, *NumberOfBlocks=0x%x.\n",
      __FUNCTION__,
      *BlockSize,
      *NumberOfBlocks));
  ...
  )
{
  ...
  FlashInstance = INSTANCE_FROM_BLKIO_THIS(This);
  Media = This->Media;
  ...
  DataOffset = GET_DATA_OFFSET (FlashInstance->RegionBaseAddress,
                 FlashInstance->StartLba + Lba,
                 FlashInstance->Media.BlockSize);

  // Read the memory-mapped data
  CopyMem (Buffer, (UINTN *)DataOffset, BufferSizeInBytes);
  ...
  )
{
  ...
  EfiConvertPointer (0x0, (VOID**)&mFvbDevice->BlockIoProtocol.FlushBlocks);
  EfiConvertPointer (0x0, (VOID**)&mFvbDevice->BlockIoProtocol.ReadBlocks);
  EfiConvertPointer (0x0, (VOID**)&mFvbDevice->BlockIoProtocol.Reset);
  EfiConvertPointer (0x0, (VOID**)&mFvbDevice->BlockIoProtocol.WriteBlocks);
  ...
  )
{
  ...
  // Read SPI flash ID
  Status = SpiFlashProtocol->ReadId (&FlashInstance->SpiDevice);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  FlashInstance->BlockIoProtocol.Media = &FlashInstance->Media;
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (&FlashInstance->Handle,
                  ...
                  &gEfiBlockIoProtocolGuid,  &FlashInstance->BlockIoProtocol,
