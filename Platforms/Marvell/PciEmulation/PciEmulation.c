  ...
  )
{
  ...
  // recognize device and fill proper BAR
  //
  ClassCodes = (ClassCode1 << 16) + (ClassCode2 << 8) + ClassCode3;

  switch (ClassCodes) {
  case XHCI_PCI_CLASS_CODE_NR:
    Private->ConfigSpace->Device.Bar[EFI_XHCI_BAR_INDEX] = Private->RootBridge.MemoryStart;
    break;
  case AHCI_PCI_CLASS_CODE_NR:
    ...
    break;
  default:
    DEBUG((DEBUG_ERROR, "PciEmulation: Unknown PCI device. Abort.\n"));
    return EFI_INVALID_PARAMETER;
  }
