  ...
  )
{
  ...
  if (Header->Magic != MAIN_HDR_MAGIC) {
    DEBUG ((DEBUG_ERROR, "%a: Bad Image magic 0x%08x != 0x%08x\n", __FUNCTION__, Header->Magic, MAIN_HDR_MAGIC));
    return EFI_DEVICE_ERROR;
  }
  ...
  if (Checksum != ChecksumBackup) {
    DEBUG ((DEBUG_ERROR, "%a: Bad Image checksum. 0x%x != 0x%x\n", __FUNCTION__, Checksum, ChecksumBackup));
    return EFI_DEVICE_ERROR;
  }
